    ...
    const generateInterviewAnswer = async (question: string) => {
      try {
        const response = await fetch('/api/generate-suggestion', {
          ...
          body: JSON.stringify({
            context: `Question: ${question}\n\nGenerate a brief, 1-2 sentence answer for this interview question. Use the candidate's background: ${interviewConfig.candidate.experience.map(exp => `${exp.role} at ${exp.company}`).join(', ')}. Keep it short and specific.`,
        ...
        if (response.ok) {
          ...
          if (data.suggestions && Array.isArray(data.suggestions) && data.suggestions.length > 0) {
            return data.suggestions[0]; // Return the first suggestion
          }
          return 'I would provide a thoughtful response based on my experience and the specific question asked.';
        }
      } catch (error) {
